  ...
  function BindAngularDirectiveCtrl($scope, $filter, $http, $compile, DTOptionsBuilder, DTColumnBuilder)
  {
     ...
     vm.dtOptions  = DTOptionsBuilder.fromSource('/index.cfm/costing_version/getCosting_Version'+window.location.search)
      ...
      vm.dtColumns = [
          ...
          DTColumnBuilder.newColumn('CV_WASTE').withTitle('WASTE (%)'),
          DTColumnBuilder.newColumn('CV_MARGIN').withTitle('MARGIN (%)'),
          DTColumnBuilder.newColumn(null).withTitle('FACTORY COST').renderWith(numberCv_fty),
          DTColumnBuilder.newColumn('CV_WEIGHT').withTitle('WEIGHT (gr)'),
          DTColumnBuilder.newColumn('CV_VOLUME').withTitle('VOLUME (cm3)'),
