  	...
  	function BindAngularDirectiveCtrl($scope, $filter, $http ,$compile, DTOptionsBuilder, DTColumnBuilder)
  	{
		...
		vm.dtOptions_detail  = DTOptionsBuilder.fromSource('/index.cfm/price_list_factory_detail/getPLF_detail?id=' + getQueryVariable('id'))
     	...
     	vm.dtColumns_detail  = [
			...
			DTColumnBuilder.newColumn('FAC_COST').withTitle('COST').renderWith(formatNumber).withClass("text-right"),
			DTColumnBuilder.newColumn('CURR_CODE_FAC').withTitle('CURRENCY'),

			DTColumnBuilder.newColumn('FTY_CURR').withTitle('SELL CALC').renderWith(formatNumber).withClass("text-right"),
			DTColumnBuilder.newColumn('CURR_CODE_FAC').withTitle('CURRENCY'),

			DTColumnBuilder.newColumn('PL_CURR').withTitle('SELL CALC').renderWith(formatNumber).withClass("text-right"),
			DTColumnBuilder.newColumn('CURR_CODE').withTitle('CURRENCY'),

			DTColumnBuilder.newColumn('PLFD_FTY_SELL_3').withTitle('SELL PRICE').renderWith(editsell3).withClass("text-right"),
			DTColumnBuilder.newColumn('CURR_CODE').withTitle('CURRENCY'),
			DTColumnBuilder.newColumn(null).withTitle('').renderWith(actionsHtml).notSortable().withOption('width',"2%")
      	];
