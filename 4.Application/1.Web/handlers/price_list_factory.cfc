component output="false" displayname=""  {
	...
	function getPLF(event,prc,rc)
	{
		...
		if((usertype eq 1) or (usertype eq 2))
		{
			...
			var fty_cc = queryExecute("select * from currency_convert where id_currency = #factory.getCurrency().getid_currency()# order by cc_date desc limit 1");
			...
			for(item in plfList) {
			   ...
			   plf.ex_Rate       = numberFormat(item.getplf_Ex_Rate(),"9.99");
			   ...
			   plf.creation_date = plf_curr.recordcount != 1 ?"":numberFormat(plf_curr.cc_value,"9.99");
			   plf.plfupdate     = dateformat(item.getplf_update(), "dd/mm/yy");
	...
	{
		var costing_season =  queryExecute("select distinct cost_season from costing order by cost_season");
		var listseason = [];
	...
	private void function insertCopy(any newseason,any price_factory, any zone, any plf_code, any new_description) {
		if(isNULL(price_factory[1])){
			return event.renderData(type="json",data={ 'success' : false , 'message' : 'Add new price list factory failed. The price to copy does not exist !' });
		}else{
			var itemzone = price_factory[1].getzone();
			var plf_date = LSDateTimeFormat(Now(),'dd/mm/yyyy');
			...
			newplf.setplf_correction(price_factory[1].getplf_correction());
			newplf.setplf_Ex_Rate(price_factory[1].getplf_Ex_Rate());
			newplf.setplf_date(plf_date);
			newplf.setplf_update(LSDateTimeFormat(Now(),'dd/mm/yyyy'));
			newplf.setfactory (price_factory[1].getFactory());
			newplf.setzone(itemzone);
			newplf.setcurrency(price_factory[1].getCurrency());
			newplf.setuser_created(userService.getLoggedInUser());
			newplf.setlanguage(price_factory[1].getLanguage());
			var ftycurr = isNull(newplf.getFactory())?javaCast("null",""):newplf.getFactory().getCurrency();
			entitySave(newplf);
			//set price list factory detail
			var oldplfdet = entityLoad("price_list_factory_detail", {price_list_factory: price_factory[1]});
			for(olditem in oldplfdet) {
				var newplfdet = entityNew("price_list_factory_detail");
				var costing   = olditem.getcosting();
				var costing_v = olditem.getcosting_version();
				newplfdet.setCosting(olditem.getCosting());
				newplfdet.setCosting_version(olditem.getCosting_version());
				var fty_cc = currencyConvertService.getCC_byDate(ftycurr.getId_currency(),dateformat(plf_date,"yyyy/mm/dd"));
				var fty_curr_rate = fty_cc.cc_value;
				var plfd_fty_cost_0 = costing_v.getCv_fty_cost_0();
				var plfd_fty_sell_1 = plfd_fty_cost_0 + plfd_fty_cost_0*newplf.getplf_correction()/100;
				var plfd_fty_sell_2 = plfd_fty_sell_1*price_factory[1].getplf_Ex_Rate()/fty_curr_rate;
				var plfd_fty_sell_3 = plfd_fty_sell_2;
				...
				newplfdet.setcreated(LSDateTimeFormat(Now(),'dd/mm/yyyy'));
				newplfdet.setupdated(LSDateTimeFormat(Now(),'dd/mm/yyyy'));
				...
				newplfdet.setfactory(olditem.getfactory());
				newplfdet.setcurrency(olditem.getCurrency());
				entitySave(newplfdet);
			}
		}
	}
	...
	public any function CopyDataBlank(event,rc,prc) {
		if(event.GETHTTPMETHOD() == "POST")
		{
			// transaction {
				try {
					var priceFactory = entityLoad("price_list_factory", {id_plf: sourceplf, factory: userService.getLoggedInUser().getFactory()});
					var textplfdes = rc.text_desplf;
					if(rc.sourcezone != '') {
						var zone = entityLoad("zone", {id_Zone: rc.sourcezone}, true);
						insertCopy(rc.desseason, priceFactory, zone, textplfdes, rc.new_description);
					}
					// transactioncommit();
					event.renderData(type="json",data={ 'success' : true , 'message' : 'Copied price list factory successfully'});
				}catch(type variable) {
					// transactionrollback();
