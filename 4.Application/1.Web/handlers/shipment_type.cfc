component output="false" {
	...
	public function addNew(event,rc,prc){
		if(event.GETHTTPMETHOD() == "POST")
		{
			if(rc.id_shipment_type == 0){
				...
				var newshipt = shipmenttypeService.new({ict_code:rc.code, ict_description:rc.description,updated:created,created:created,user_created:user_created,user_updated:user_created});
				...
				var shipt = shipmenttypeService.get(id=rc.id_shipment_type,true);
				var user_updated = userService.getLoggedInUser();
				var updated = now();

				shipt.setUser_Updated(user_updated);
				shipt.setUpdated(updated);
				shipt.setict_code(rc.code);
				shipt.setict_description(rc.description);
	...
	public any function delete(event, rc, prc) {
		...
		if(event.GETHTTPMETHOD){
			var getshipt = shipmenttypeService.get(id=rc.sid);
