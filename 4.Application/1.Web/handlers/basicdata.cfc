...
component output="false" displayname="basicdata" extends="adminHandler"  {
	...
	function getcustomer(event,prc,rc)
	{
		...
		if(structKeyExists(rc, "idagent")){
			var customerList = EntityLoad('customer',{agent:EntityLoadByPK("agent",rc.idagent)});
		}else if(userData.getIs_root() == 1){
		...
		for(item in customerList)
		{
			...
			// customer.code= item.getcs_code();
			customer.name = item.getcs_name();
			...
			// customer.zone = isNull(isNull(item.getAgent())?"": item.getAgent().getZone())?"":item.getAgent().getZone().getz_code();
			// customer.zoneid = isNull(isNull(item.getAgent())?"": item.getAgent().getZone())?"":item.getAgent().getZone().getid_Zone();
