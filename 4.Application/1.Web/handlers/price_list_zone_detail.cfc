	...
	function getplz(event,prc,rc){
		if(event.GETHTTPMETHOD() == "POST")
		{
			...
			plz={
				...
				"plf_code"        : isNull(c_plz.getPrice_list_factory())?"":c_plz.getPrice_list_factory().getId_plf(),
		...
		if(event.GETHTTPMETHOD() == "GET"){
			...
			for(item in plz_detailList) {
			   	...
			   	var plz_detail         			= {};
					...
					plz_detail.plzd_weight      = isNull(item.getPlzd_weight())?"":item.getPlzd_weight();
					...
					var plfCurr 				= isNull(plf)    ?null:plf.getCurrency();
	...
	function updatePLZ(event,rc,prc){
		if(event.GETHTTPMETHOD() == "POST"){
			if(rc.id_plz neq 0){
				...
				var plf = PLFService.get(id=rc.id_plf);
	...
	function addEdit(event,rc,prc){
		if(event.GETHTTPMETHOD() == "POST"){
			if(rc.id_plz_det == 0){
				...
				plz_det.setPlzd_zone_sell_6(rc.plzd_zone_sell_6);
				plz_det.setPlzd_pvpr_8(rc.plzd_pvpr_8);
