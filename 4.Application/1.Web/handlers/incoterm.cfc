component output="false" {
	...
	public function addNew(event,rc,prc){
		if(event.GETHTTPMETHOD() == "POST")
		{
			if(rc.id_incoterm == 0){
				...
				var ico = incotermService.get(id=rc.id_incoterm,true);
				...
				ico.setfty_code(rc.code);
				ico.setfty_description(rc.description);
				ico.setcurrency(rc.currency);
				ico.setcontact(rc.contact);
				ico.setlanguage(rc.language);
	...
	public any function delete(event, rc, prc) {
		...
		if(event.GETHTTPMETHOD){
			var getico = incotermService.get(id=rc.iid);
