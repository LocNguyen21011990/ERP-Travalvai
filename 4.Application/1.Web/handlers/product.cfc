...
component output="false" displayname=""  {
	...
	function getProductDetail(component productItem){
		...
		var agentPriceCus 	 = 0;
		var finalPriceCus 	 = 0;
		var factoryPriceManual = isNull(productItem.getPr_fty_sell_9()) ? 0 : NumberFormat(productItem.getPr_fty_sell_9(),numberFormat);
		var agentPriceManual   = isNull(productItem.getPr_zone_sell_10()) ? 0 : NumberFormat(productItem.getPr_zone_sell_10(),numberFormat);
		var finalPriceManual   = isnull(productItem.getPr_PVPR_11()) ? 0 : NumberFormat(productItem.getPr_PVPR_11(),numberFormat);
		var product            = {};
		...
		product.garmentCode    = project.getId_Project()&"-"&(isNull(productItem.getPattern())?"":isNull(productItem.getPattern().getId_Pattern()) ? "":productItem.getPattern().getId_Pattern())&"-"&(isNull(productItem.getPattern_variantion())?"":isNull(productItem.getPattern_variantion().getId_pattern_var())?"":productItem.getPattern_variantion().getId_pattern_var());
		...
		product.price = {
			"sell_4" = isnull(plz_det) ? 0 : NumberFormat(plz_det.getPlzd_Fty_Sell_4(),numberFormat),
			"sell_6" = isnull(plz_det) ? 0 : NumberFormat(plz_det.getPlzd_zone_sell_6(),numberFormat),
			"pvpr_8" = isnull(plz_det) ? 0 : NumberFormat(plz_det.getPlzd_pvpr_8(),numberFormat)
		};
		...
		if(!isNull(productCust)){
			for(item in productCust){
				...
				factoryPriceCus += NumberFormat(quantity*plz_detail.getplzd_fty_sell_4(),numberFormat);
				agentPriceCus   += NumberFormat(quantity*plz_detail.getplzd_zone_sell_6(), numberFormat);
				finalPriceCus   += NumberFormat(quantity*plz_detail.getplzd_pvpr_8(), numberFormat);
			}
		}
		product.customize = {
	...
	function calFinalPrice(numeric priceList, numeric manualPrice, numeric customPrice, numeric idcontract){
		...
		if(manualPrice == 0){
			if(priceList > 0){
				finalPrice = NumberFormat(priceList + customPrice,numberFormat);
			}else{
				finalPrice = 0;
			}
		}else{
			if(idcontract == 0){
				finalPrice = NumberFormat(manualPrice + customPrice, numberFormat);
			}else{
				...
				for(var i =0; i < noOfYear; i++){
					...
					if( (start < currentDay) && (currentDay < end)){
						finalPrice = NumberFormat((((1 + (increase_year/100)) ^ i) * manualPrice + customPrice), numberFormat);
						return finalPrice;
					}
				}
			}
		...
		if(event.GETHTTPMETHOD() == "POST")
		{
			if(rc.id_Type== 0)
			{
				...
				for(item in dataDes){
					...
					group_language.save(group_type);
				}
	...
	public any function addNewGroup(event,rc,prc) {
		if(event.GETHTTPMETHOD() == "POST")
		{
			...
			if(rc.id_Group== 0)
			{
				...
				var zone = product_groupService.get(rc.id_Group);
				var user_updated = userService.getLoggedInUser();

				zone.setUser_Updated(user_updated);
				zone.setUpdated(created);
				zone.setGp_code(UCase(rc.code));

				var dataDes = deserializeJSON(rc.des);
				var result = product_groupService.save(zone);
				for(item in dataDes){
					if(!structKeyExists(dataDes[item], "description")){
						dataDes[item].description = "";
					}
					var group_lang = EntityLoad("group_product_language", {group = zone, language = EntityLoadByPK("languages", dataDes[item].id_language)}, true);
	...
	public function getZone(){
		...
		if(userType == 1){
			...
			for(item in zone){
				...
				struZoneInfo["z_code"]	= item.getz_code();
	...
	public function getAgent(){
		...
		if(event.GETHTTPMETHOD() == "POST"){
			if(userType == 1){
				...
				for(item in agent){
					...
					struAgentInfo["ag_code"]	= item.getag_code();
		...
		if(userType == 2){
			...
			for(item in aAgentID){
				...
				struAgentInfo["ag_code"]	= agent.getag_code();
				arrayAppend(aAgentInfo, struAgentInfo);
			}
			event.renderData(type="json",data=aAgentInfo);
		}
	...
	public function getplz_id(event, rc, prc) {
		...
		// if(event.GETHTTPMETHOD() == "POST"){
			if(usertype neq 4){
				...
				var aProject = productService.getProject(zoneID, agentID);
				if(!isnull(aProject)) {
	...
	public function getcost_code(event,rc,prc) {
		...
		if(usertype neq 4){
			if(structKeyExists(rc, "plz_id")){
				...
				var costing = productService.getcode_cost();
			}
	...
	public function getPattern(event, rc, prc) {
		...
		if((usertype neq 4))
		{
			var typeProduct = EntityLoadByPK("type_of_products", rc.tp_id);
			var patterns = entityLoad("patterns", {group_of_product : typeProduct.getgroup_of_product()});
			if(!isnull(patterns)) {
				for(item in patterns) {
					var str = {};
					str["id_pattern"] = item.getid_pattern();
					str["pt_code"] = item.getpt_code();
	...
	public function addNewProduct(event, rc, prc) {
		...
		if(event.GETHTTPMETHOD() == "POST")
		{
			try {
				...
				var newProduct = entityNew("product");
				if(userType != 4){
			...
			if(isNumeric(rc.id_Product) and rc.id_Product >0){
				try{
					...
					product.setpr_version(rc.pr_version);
		      		...
		      		product.setProduct_status(pr_status);
	...
	public function getImagesOfProducts(event, rc, prc) {
		try {
			...
			for(item in products) {
				arrayAppend(lstimages, {"imagename": item.getPr_sketch()});
			}
