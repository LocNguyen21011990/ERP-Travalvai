...
component output="false" displayname=""  {
	...
	public any function addNew(event,rc,prc) {
		if(event.GETHTTPMETHOD() == "POST")
		{
			if(rc.id_Project== 0)
			{
				...
				project.setpj_act_1(rc.groupWaitingFD);
				project.setpj_act_2(rc.groupUrgent);
				project.setpj_act_3(rc.groupVUrgent);
				project.setpj_act_4(rc.groupSentTCom);
				project.setpj_act_5(rc.groupSentTCus);
				project.setpj_act_6(rc.groupSentTD);
	...
	public any function addNewComment(event,rc,prc) {
		if(event.GETHTTPMETHOD() == "POST")
		{
			if(rc.id_Comment == 0)
			{
				...
				var userlogged = userService.getLoggedInUser();
				comment.setUser_Updated();
				comment.setpj_com_comment(rc.comment)
				comment.setpj_com_date(rc.date)
	...
	public any function getzoneagent(event,rc,prc) {
		...
		project.id_agent = isNull(customer.getagent())?"":customer.getagent().getid_Agent();
		project.name_agent = isNull(customer.getagent())?"":customer.getagent().getag_code();
 		project.name_zone = isNull(isNull(customer.getAgent())?"": customer.getAgent().getZone())?"":customer.getAgent().getZone().getz_code();
   		project.id_zone = isNull(isNull(customer.getAgent())?"": customer.getAgent().getZone())?"":customer.getAgent().getZone().getid_Zone();
	...
	function index()
	{
		...
		for(item in projectList) {
		 	var project = {};
		 	var customer = isNull(item.getcustomer())?"":item.getcustomer();
		 	...
		 	project.id_agent = isNull(item.getcustomer())?"":customer.getagent().getid_Agent();
		   	project.agent = isNull(isNull(item.getCustomer())?"": item.getCustomer().getAgent())?"": item.getCustomer().getAgent().getag_code();
		 	project.id_zone = isNull(item.getcustomer())?"":customer.getagent().getzone().getid_Zone();
		   	project.zone = isNull(isNull(isNull(item.getCustomer())?"": item.getCustomer().getAgent())?"": item.getCustomer().getAgent().getZone())?"":item.getCustomer().getAgent().getZone().getz_code();
	...
	function getProj_product(event,rc,prc){
		if(event.GETHTTPMETHOD()=="GET"){
			...
			for(item in proj_prod){
				var patternid = isNull(item.getPattern())?"":item.getPattern().getId_pattern();
				var pattern_var_id = isNull(item.getPattern())?"":item.getPattern().getId_pattern();
				var newp = {
					  "id_product"             = item.getId_product()
					, "Garment_code"           = item.getProject().getId_Project() & " - " & patternid & " - " & pattern_var_id
	...
	function getcomment(event,rc,prc)
	{
		if(structKeyExists(URL, "idproject")){
			var commentList = EntityLoad('project_comment', {project=EntityLoadByPK("project",URL.idproject)});
			var comments = [];
	...
	function deleteProject(event, rc, prc) {
		try {
			...
			entityDelete(project);
			ormflush();
			event.renderData(type="json",data={ 'success' : true , 'message' : 'Delete project success' });
		}
		catch(any ex) {
			event.renderData(type="json",data={ 'success' : false , 'message' : 'Project in used. Can not delete' });
		}
