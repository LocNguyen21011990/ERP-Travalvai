...
//			result.plz = isNull(resultQuery.getprice_list_zone())?0:resultQuery.getprice_list_zone().getid_plz();
		}
	...
	function addNew(event,prc,rc)
	{
		if(event.GETHTTPMETHOD() == "POST")
		{
			if(rc.id_Agent== 0)
			{
				...
				var newAgent = agentService.new({ag_code:rc.code,ag_description:rc.description,ag_commission:rc.commission,language:rc.language,updated:created,created:created});
